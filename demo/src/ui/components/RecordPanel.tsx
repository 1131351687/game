// 记录面板（E3 城邦时代 · 刻录系统）
//
// 刻录 = 把"口头知识"刻上泥板，使其效果从 ×50% 提升到 ×100%。
// 占用记录槽位（硬上限），不可撤销；每刻录一项科技，档案库加成生效。
//
// 本组件对标 E1 的 FireDashboard（火种仪表盘）：紧凑横向条状布局，
// 深色简约风格。结构：
//   📜 记录 [线性进度条] 2/3  ✍️ 书吏 2  档案加成 ×1.06
//   下面是可刻录科技列表，每项一个「刻录」按钮。
//
...
export function RecordPanel({ className }: { className?: string }) {
  ...
  const archiveTotal = 1 + eff.archiveBonus * state.recordedOnce.length;
