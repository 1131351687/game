...
import { INITIAL_STATE, LOOP, E2, E4 } from '../data/constants';
...
export interface GameState {
  ...
  /**
   * 版图扩张（E4）：n → n+1。
   * 消耗 铸币 800·n^1.3 + 铁 200·n^1.1，进入 (15+2n) 秒平定期（A_eff ×1.5）。
   * 返回 null = 成功；字符串 = 失败原因（供 UI 直接展示）。
   */
  expandTerritory: () => string | null;
  /** 切换政体（E4）。返回 null = 成功；字符串 = 失败原因。 */
  switchPolity: (next: 'monarchy' | 'republic' | 'theocracy') => string | null;
}
...
function engineView(s: GameState): engine.EraState {
  return {
    ...
    polity: s.polity,
    polityCooldownSec: s.polityCooldownSec,
    expansionFlatSec: s.expansionFlatSec,
    successionTimer: s.successionTimer,
...
export const useStore = create<GameState>((set, get) => ({
  ...
  doTick: (dt) => {
    ...
    set({
      ...
      coin: r.coin,
      order: r.order,
      polityCooldownSec: r.polityCooldownSec,
      expansionFlatSec: r.expansionFlatSec,
      successionTimer: r.successionTimer,
    });
  ...
  loadSnapshot: (data) => {
    ...
    const migrated: Partial<GameState> = {
      ...
      // v7 起：E4 状态字段（旧档一律补默认值；跃迁只新增不重置）
      iron: data.iron ?? 0,
      coin: data.coin ?? 0,
      order: data.order ?? 70,
      territory: data.territory ?? 1,
      polity: data.polity ?? null,
      polityCooldownSec: data.polityCooldownSec ?? 0,
      expansionFlatSec: data.expansionFlatSec ?? 0,
      successionTimer: data.successionTimer ?? 0,
  ...
  advanceEra: () => {
    ...
    set({
      ...
      // E3 字段（E1→E2 交接置空；E2→E3 采用 transition 的随机矿藏）
      copper: 0,
      tin: 0,
      bronze: 0,
      lapis: 0,
      recorded: [],
      recordedOnce: [],
      // ⚠️ 修复：此前无条件写 'alluvial'，把 transition 的随机矿藏覆盖掉了
      // （浏览器端永远冲积平原、模拟器却正常——因为模拟器直调 transition）
      localOre: t.localOre,
      tradeRoutes: [],
      reputation: 50,
      // ── E4 字段（跃迁只新增不重置：进入 E4 时初始化一次，其余时代原样保留）──
      iron: nextEraId === 'E4' ? 0 : s.iron,
      coin: nextEraId === 'E4' ? 0 : s.coin,
      order: nextEraId === 'E4' ? 70 : s.order,
      territory: nextEraId === 'E4' ? 1 : s.territory,
      polity: nextEraId === 'E4' ? null : s.polity,
      polityCooldownSec: 0,
      expansionFlatSec: 0,
      successionTimer: 0,
      // 首次进入 E4 → 解锁 P1 文明重启
      p1Unlocked: nextEraId === 'E4' ? true : s.p1Unlocked,
      legacyPoints: s.legacyPoints,
    });
  ...
  expandTerritory: () => {
    const s = get();
    const check = empire.canExpand(engineView(s));
    if (!check.ok) return check.reason ?? '无法扩张';

    const n = s.territory;
    set({
      territory: n + 1,
      coin: s.coin - check.coinCost,
      iron: s.iron - check.ironCost,
      expansionFlatSec: check.flatSec,
    });
    get().addMessage(
      `版图扩张至 ${n + 1} 格（铸币 −${check.coinCost} / 铁 −${check.ironCost}，平定期 ${check.flatSec}s）`,
      'event',
      true
    );
    return null;
  ...
  switchPolity: (next) => {
    const s = get();
    const check = empire.canSwitchPolity(engineView(s), next);
    if (!check.ok) return check.reason ?? '无法切换政体';

    const prev = s.polity;
    set({
      polity: next,
      coin: s.coin - E4.GOV_SWITCH_COIN_COST,
      order: Math.max(0, s.order - E4.GOV_SWITCH_ORDER_COST),
      polityCooldownSec: E4.GOV_SWITCH_COOLDOWN_SEC,
      // 换政体重置继承危机计时（只有君主制继续计时）
      successionTimer: next === 'monarchy' ? s.successionTimer : 0,
    });
    get().addMessage(
      `政体变更：${prev ? POLITY_NAME[prev] : '无'} → ${POLITY_NAME[next]}（铸币 −${E4.GOV_SWITCH_COIN_COST}，秩序 −${E4.GOV_SWITCH_ORDER_COST}）`,
      'event',
      true
    );
    return null;
...
/** 政体中文名（消息文案用） */
const POLITY_NAME: Record<'monarchy' | 'republic' | 'theocracy', string> = {
  monarchy: '君主制',
  republic: '共和制',
  theocracy: '神权制',
};
